    ...
    <div class="container-fluid">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <a href="<?php echo site_url($pagelink) ?>"><button class="btn btn-primary" type="button">Reset</button></a>
                ...
                <div class="col-lg-2 pull-right">
                    <div class="seipkon-breadcromb-right">
                        <?php if ($logged_user_type == 1 || has_perm($logged_user_id, $modu, $submod, 'Add')) { ?>
                            <a href="<?php echo base_url('Inventory/add_storeout/') . $pagetype ?>" class="btn btn-info btn-vsm"><i class="fa fa-plus-circle"></i> Add New</a>
                        <?php } ?>
                    </div>
                </div>
            </div>
        ...
        <div class="page-box">
            <div class="advance-table table-overflow">
                <table id="storeout_tbl" class="my_custom_datatable table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th>SN</th>
                            <th>No</th>
                    ...
                    <tbody>
                        <?php
                        $i = 1;
                        if (!empty($storeout_value)) {
                            foreach ($storeout_value as $value) {
                            
                        ?>
                                <tr>
                                    <td><?= $i; ?></td>
                                    <td><?= $value->ivt_stkout_no; ?></td>
                                    <td><?= date('d-m-Y', strtotime($value->ivt_stkout_date)); ?></td>
                                    <td><?= $value->m_company_name; ?></td>
                                    <td><?= $value->m_godown_name; ?></td>
                                    <td><?= $value->totqty; ?></td>
                                    <td><?= $value->ivt_stkout_remark; ?></td>
                                    <td><?= $value->m_admin_name; ?></td>

                                    <td class="wd-30">
                                        <button type="button" class="btn btn-primary btn-action" onclick="viewmodalfun(<?php echo $value->ivt_stkout_no; ?>);" title="View"><i class="fa fa-eye"></i></button>
                                        <?php if ($logged_user_type == 1 || has_perm($logged_user_id, $modu, $submod, 'Edit')) { ?>
                                            <a href="<?php echo base_url('Inventory/add_storeout/' . $pagetype . '?editid=') . $value->ivt_stkout_no; ?>" class="btn btn-info btn-action" title="Edit" data-toggle="tooltip"><i class="fa fa-edit"></i></a>
                                        <?php }
                                        if ($logged_user_type == 1 || has_perm($logged_user_id, $modu, $submod, 'Delete')) { ?>
                                            <button class="btn btn-danger btn-action delete-storeout" data-dtype="1" data-value="<?php echo $value->ivt_stkout_no; ?>" title="Delete" data-toggle="tooltip"><i class="fa fa-trash"></i></button>
                                        <?php } ?>
                                    </td>
                                </tr>
                        <?php
...
<script>
    function viewmodalfun(stkno) {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Inventory/get_storeout_dtl'); ?>",
            data: {
                stkno
            },
            dataType: "JSON",
            success: function(data) {
                if (data != '') {
                    ...
                    $.each(data.ivt_stkout_items, function(i, item) {
                        tabbody += ` <tr>
                                                <td>` + (i + 1) + `</td>
                                                <td>` + item.m_product_name + `</td>
                                                <td>` + item.productsize + `</td>
                                                <td>` + item.productunit + `</td>
                                                <td>` + item.ivt_stkout_prodqty + `</td>
                                                <td>` + item.ivt_stkout_prodrate + `</td>
                                                <td>` + item.ivt_stkout_tamt + `</td>
                                            </tr>`;
                    });
                    ...
                    $('#modalcontent').html(`<div class="row">
                            ...
                            <div class="col-md-12 col-xs-8">
                                <b>Remark :</b> ` + data.ivt_stkout_remark + `
                            </div>
